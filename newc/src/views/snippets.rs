...
use iced::widget::{Space, button, column, pane_grid, row, scrollable, text};
use iced::{Element, Length};

/// Identifies which pane of the Snippets resizable pane-grid is being rendered.
#[derive(Clone, Copy)]
pub enum SnippetsPane {
    List,
    Code,
}
...
const CATEGORIES: &[SnippetCat] = &[
    SnippetCat {
        ...
        items: &[
            ...
            s!(
                "if / else if / else",
                "Branch on multiple conditions.",
                "if (condition) {\n    \n} else if (other) {\n    \n} else {\n    \n}"
            ),
            s!(
                "switch",
                "Dispatch on an integer or enum value.",
                "switch (var) {\n    case 1:\n        break;\n    default:\n        break;\n}"
            ),
        ],
    },
    SnippetCat {
        ...
        items: &[
            ...
            s!(
                "array init",
                "Declare and zero-initialise an array.",
    ...
    SnippetCat {
        ...
        items: &[
            s!(
                "pointer basics",
                "Declare, address, dereference.",
                "int x = 42;\nint *ptr = &x;\nprintf(\"%d\\n\", *ptr);"
            ),
            s!(
                "malloc / free",
                "Heap allocate and release.",
    ...
    SnippetCat {
        ...
        items: &[
            s!(
                "open / close file",
                "Open a file, check for errors, then close.",
    ...
    SnippetCat {
        ...
        items: &[
            ...
            s!(
                "strtok_r",
                "Split a string by delimiter (re-entrant).",
                "char *saveptr;\nchar *token = strtok_r(str, \",\", &saveptr);\nwhile (token) {\n    /* process token */\n    token = strtok_r(NULL, \",\", &saveptr);\n}"
            ),
            s!(
                "string to int (safe)",
                "Convert string to integer with error check.",
    ...
    SnippetCat {
        ...
        items: &[
            ...
            s!(
                "errno check",
                "Inspect errno after a failed syscall.",
                "#include <errno.h>\nif (errno == ENOENT) {\n    fprintf(stderr, \"File not found\\n\");\n} else if (errno == EACCES) {\n    fprintf(stderr, \"Permission denied\\n\");\n}"
            ),
            s!(
                "realloc safe",
                "Reallocate without losing original pointer on failure.",
    ...
    SnippetCat {
        ...
        items: &[
            s!(
                "qsort integers",
                "Sort an integer array with qsort.",
                "int cmp_int(const void *a, const void *b) {\n    return (*(int*)a - *(int*)b);\n}\nqsort(arr, n, sizeof(int), cmp_int);"
            ),
            s!(
                "qsort strings",
                "Sort a string array with qsort.",
                "int cmp_str(const void *a, const void *b) {\n    return strcmp(*(const char**)a, *(const char**)b);\n}\nqsort(arr, n, sizeof(char*), cmp_str);"
            ),
            s!(
                "bubble sort",
                "Simple O(n²) sort for small arrays.",
    ...
    SnippetCat {
        name: "Bit Ops",
        items: &[
            s!("set bit", "Set bit N in a value.", "value |= (1 << N);"),
            s!(
                "clear bit",
                "Clear bit N in a value.",
                "value &= ~(1 << N);"
            ),
            s!("toggle bit", "Flip bit N in a value.", "value ^= (1 << N);"),
            s!(
                "test bit",
            ...
            s!(
                "is power of two",
                "Test if a positive integer is a power of 2.",
    ...
    SnippetCat {
        ...
        items: &[
            ...
            s!(
                "traverse list",
                "Walk every node in a singly-linked list.",
                "Node *cur = head;\nwhile (cur) {\n    /* process cur->data */\n    cur = cur->next;\n}"
            ),
            s!(
                "free list",
                "Release all nodes in a list.",
                "while (head) {\n    Node *tmp = head;\n    head = head->next;\n    free(tmp);\n}"
            ),
        ],
    },
    SnippetCat {
        ...
        items: &[
            s!(
                "sigaction handler",
                "Install a signal handler with sigaction.",
                "#include <signal.h>\nvoid handler(int sig) {\n    /* handle signal */\n}\nstruct sigaction sa = {0};\nsa.sa_handler = handler;\nsigemptyset(&sa.sa_mask);\nsigaction(SIGINT, &sa, NULL);"
            ),
            s!(
                "volatile flag",
                "Safe flag set by a signal handler.",
...
pub fn view(state: &crate::state::AppState) -> Element<'_, crate::state::Message> {
    ...
    // Category tabs
    let cat_btns: Vec<Element<Message>> = CATEGORIES
        .iter()
        .enumerate()
        .map(|(i, cat)| {
    ...
    // Snippet list
    let snippet_btns: Vec<Element<Message>> = cat
        .items
        .iter()
        .enumerate()
        .map(|(i, snippet)| {
    ...
    let list_cell = std::cell::RefCell::new(Some(
        scrollable(column(snippet_btns).spacing(6))
            .height(Length::Fill)
            .into(),
    ));
    let code_cell =
        std::cell::RefCell::new(Some(scrollable(code_panel).height(Length::Fill).into()));
    ...
    column![header_row, cat_row, grid,]
        .spacing(10)
        .padding(12)
        .into()
}
