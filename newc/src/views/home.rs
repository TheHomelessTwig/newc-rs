...
pub fn view(state: &AppState) -> Element<'_, Message> {
    ...
    let mut ws_row: Vec<Element<Message>> = vec![
        button(text("All").size(12))
            ...
            .style(if all_active { th::btn_nav_active } else { th::btn_nav_inactive })
            .into(),
    ];
    ...
    ws_row.push(
        button(text("Archived").size(12))
            ...
            .style(if state.show_archived { th::btn_nav_active } else { th::btn_nav_inactive })
            .into(),
    );
    ws_row.push(
        button(text("+").size(12))
        ...
        Some(
            container(
                column![
                    ...
                    th::subheading("Scaffold, build, and manage C projects with reusable function libraries."),
