...
use iced::widget::{
    Space, button, column, pane_grid, row, scrollable, text, text_editor, text_input,
};
...
pub fn view<'a>(state: &'a AppState, lib: &'a FunctionLibrary) -> Element<'a, Message> {
    ...
    let mut toolbar = row![
        ...
        text_input("Search…", &ls.search)
            ...
            .width(220),
        button(text("+ New Function").size(12)).on_press(Message::LibraryAddingNew(true)),
        button(text("Import .c…").size(12)).on_press(Message::ShowImport(true)),
    ]
    .spacing(8)
    ...
    let mut func_items: Vec<FuncItem> = candidates
        .iter()
        .map(|f| FuncItem {
            name: f.name.clone(),
            starred: f.starred,
    ...
    let fn_btns: Vec<Element<Message>> = func_items
        .into_iter()
        .map(|fi| {
            let color = if fi.selected {
                th::color::green()
            } else {
                Color::WHITE
            };
            ...
            row![
                ...
                button(text(display_name).size(12).color(color))
                    ...
                    .width(Length::Fill),
                button(text("→").size(10)).on_press(Message::ComposerAddBlock(
    ...
    let fn_panel = scrollable(if fn_empty {
    ...
    let detail: Element<Message> = if ls.adding_new {
        ...
        if let Some(f) = lib.all().iter().find(|f| &f.name == sel) {
            view_func(
                f,
                state.pending_library_insert_module.as_deref(),
    ...
    column![toolbar, grid,].spacing(8).padding(12).into()
}

fn view_func<'a>(
    f: &'a FunctionTemplate,
    pending_module: Option<&str>,
    font_size: f32,
) -> Element<'a, Message> {
...
fn edit_form<'a>(
    ...
    let desc_val = draft_template
        .map(|f| f.description.clone())
        .unwrap_or_default();
    let sig_val = draft_template
        .map(|f| f.signature.clone())
        .unwrap_or_default();
    let tags_val = draft_template
        .map(|f| f.tags.join(", "))
        .unwrap_or_default();
    let _notes_val = draft_template.map(|f| f.notes.clone()).unwrap_or_default();

    let title = if ls.adding_new {
        "New Function"
    } else {
        "Edit Function"
    };
    ...
    if is_valid {
        save_btn = save_btn.on_press(Message::LibrarySave(
            draft_template
                .cloned()
                .unwrap_or_else(LibraryState::new_draft),
        ));
    }
