...
pub fn view<'a>(state: &'a AppState, project: &'a Project) -> Element<'a, Message> {
    ...
    let replace_row = row![
        text_input("Replace with…", &state.search_replace)
            ...
            .width(360),
        button(text("Preview"))
            .on_press(Message::SearchReplacePreview)
            .style(th::btn_secondary),
        button(text("Apply"))
            .on_press(Message::SearchReplaceApply)
            .style(th::btn_danger),
    ]
    .spacing(8)
    .align_y(iced::Alignment::Center);
    ...
    let preview_section: Option<Element<Message>> = if state.replace_preview.is_empty() {
        ...
        let rows: Vec<Element<Message>> = state
            ...
            .map(|p| {
                column![
                    text(format!("{}:{}", p.file, p.line_no))
                        .size(11)
                        .color(th::color::text_dim()),
                    text(format!("- {}", p.before))
                        .size(12)
            ...
            .collect();
        Some(scrollable(column(rows).spacing(6)).height(200).into())
    };
    ...
    if state.search_query.is_empty() {
        return column![
            header,
            search_row,
            text("Enter a query and press Enter or click Search.").color(th::color::text_dim()),
        ]
        .spacing(10)
        .padding(16)
        .into();
    ...
    let result_rows: Vec<Element<Message>> = state
        ...
        .map(|result| {
            ...
            .on_press_maybe(result.module.as_ref().map(|m| Message::DiagJumpTo {
                module: m.clone(),
                line,
            }));
...
fn highlight_match<'a>(text_str: &str, query: &str) -> Element<'a, Message> {
    ...
    rich_text([TSpan::<()>::new(text_str.to_string())
        .font(iced::Font::MONOSPACE)
        .size(12)
        .color(th::color::text())])
    .into()
