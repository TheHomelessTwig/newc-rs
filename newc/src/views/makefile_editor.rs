...
pub fn view<'a>(state: &'a AppState, project: &'a Project) -> Element<'a, Message> {
    ...
    let header = row![
        ...
        button(text("Reset"))
            ...
            .style(th::btn_ghost),
        button(text("Save"))
            .on_press(Message::MakefileSave)
            .style(th::btn_primary),
        button(text("▶ all"))
            .on_press(Message::BuildStart("all".into()))
            .style(th::btn_secondary),
        button(text("▶ run"))
            .on_press(Message::BuildStart("run".into()))
            .style(th::btn_secondary),
        button(text("▶ clean"))
            .on_press(Message::BuildStart("clean".into()))
            .style(th::btn_secondary),
    ]
    .spacing(8)
    ...
    let body: Element<Message> = match project.build_system {
        BuildSystem::Make => {
            ...
            let cflags_line = content_text
                .lines()
                .find(|l| l.trim_start().starts_with("CFLAGS") && l.contains('='))
                .unwrap_or("");

            let flag_btns: Vec<Element<Message>> = FLAGS
                .iter()
                .map(|(flag, label)| {
            ...
            let flags_panel = container(column(flag_btns).spacing(2))
                .style(th::section_style)
                .padding(6)
                .width(110);

            row![flags_panel, editor]
                .spacing(8)
                .height(Length::Fill)
                .into()
        }
    ...
    column![header, body].spacing(8).padding(12).into()
}
