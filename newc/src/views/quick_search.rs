...
pub fn view<'a>(state: &'a AppState) -> Element<'a, Message> {
    ...
    let result_rows: Vec<Element<Message>> = results.iter().enumerate().map(|(i, result)| {
        let selected = i == qs.cursor;
        let _bg_color = if selected { Color::from_rgba(0.235, 0.392, 0.706, 0.314) } else { Color::TRANSPARENT };
    ...
    column![
        text_input("Search functions, projects… (Ctrl+P to toggle, Esc to close)", &qs.query)
...
fn collect_results(
    ...
    for path in projects {
        ...
        if q.is_empty() || name.to_lowercase().contains(&q) || path.display().to_string().to_lowercase().contains(&q) {
            results.push(QuickSearchResult::Project { name, path: path.clone() });
        }
    }
