...
pub fn view(state: &AppState) -> Element<'_, Message> {
    ...
    let valgrind_section: Option<Element<Message>> = if state.valgrind_errors.is_empty() {
        ...
        let rows: Vec<Element<Message>> = state.valgrind_errors.iter().map(|e| {
            let label = format!(
                "[{}] {}{}",
                e.kind,
                e.text,
                e.leaked_bytes.map(|b| format!(" ({b} bytes)")).unwrap_or_default()
            );
    ...
    let mut panel = column![
        row![
            ...
            Space::new().width(4),
            {
                let profile_names: Vec<String> = state.project_config.as_ref()
