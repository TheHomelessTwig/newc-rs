...
pub fn view<'a>(state: &'a AppState, project: &'a Project) -> Element<'a, Message> {
    ...
    for b in &branches {
        ...
        if b != cur_branch {
            branch_btns.push(
                button(text(b.clone()).size(11))
                    .on_press(Message::GitCheckout(b))
                    .into(),
            );
        }
    }
    ...
    let new_branch_row = row![
        ...
        button(text("Create").size(12))
            .on_press(Message::GitCreateBranch)
    ...
    let status_block: Element<Message> = if let Some(status) = git::status(&project.root) {
        column![
            row![
                ...
                button(text("Pull"))
                    .on_press(Message::GitPull)
                    .style(th::btn_secondary),
                button(text("Push"))
                    .on_press(Message::GitPush)
                    .style(th::btn_secondary),
            ]
            .spacing(8)
            .align_y(iced::Alignment::Center),
    ...
    let staged_section: Element<Message> = if staged_files.is_empty() {
        ...
        let rows: Vec<Element<Message>> = staged_files
            .iter()
            .map(|f| {
                ...
                row![
                    text(f.path.clone())
                        ...
                        .font(iced::Font::MONOSPACE)
                        .color(th::color::green()),
                    button(text("Unstage").size(10))
                        .on_press(Message::GitUnstage(path_str))
                        .style(th::btn_ghost),
                ]
                .spacing(8)
                .into()
    ...
    let unstaged_section: Element<Message> = if unstaged_files.is_empty() {
        ...
        let rows: Vec<Element<Message>> = unstaged_files
            .iter()
            .map(|f| {
                let path_str = f.path.clone();
                row![
                    ...
                    button(text("Stage").size(10))
                        .on_press(Message::GitStage(path_str))
                        .style(th::btn_secondary),
                ]
                .spacing(8)
                .into()
    ...
    let stash_btn_row = row![
        button(text("Stash").size(12))
            .on_press(Message::GitStash)
            .style(th::btn_secondary),
        button(text("Pop").size(12))
            .on_press(Message::GitStashPop)
            .style(th::btn_secondary),
    ]
    .spacing(6);
    let stash_rows: Vec<Element<Message>> = stash_entries
        .iter()
        .map(|s| {
            row![
                text(s.reference.clone())
                    .size(11)
                    .font(iced::Font::MONOSPACE)
                    .color(th::color::yellow()),
    ...
    let log_rows: Vec<Element<Message>> = log_entries
        .iter()
        .map(|entry| {
            row![
                text(entry.hash.clone())
                    .size(11)
                    .font(iced::Font::MONOSPACE)
                    .color(th::color::yellow())
                    .width(70),
